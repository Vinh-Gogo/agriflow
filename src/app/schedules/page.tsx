    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6">
        {schedules.map((schedule) => (
          <Card key={schedule.id} className="border-none shadow-sm bg-white overflow-hidden hover:ring-1 hover:ring-primary/20 transition-all">
            <CardContent className="p-0">
              <div className="flex flex-col md:flex-row md:items-center justify-between p-6 gap-6">
                <div className="flex items-center gap-4">
                  <div className={`p-4 rounded-2xl ${schedule.active ? 'bg-primary/10 text-primary' : 'bg-slate-100 text-slate-400'}`}>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                       ...
                       <Badge variant="secondary" className="text-[10px]">{schedule.zones}</Badge>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-6 border-t md:border-t-0 pt-4 md:pt-0">
                  ...
                  <div className="flex gap-2">
                    <Button variant="ghost" size="icon" className="text-slate-400 hover:text-primary"><Edit2 className="size-5" /></Button>
                    <Button variant="ghost" size="icon" className="text-slate-400 hover:text-destructive"><Trash2 className="size-5" /></Button>
                  </div>
                </div>
              </div>
            </CardContent>
      ...
      <Card className="border-dashed border-2 border-slate-200 bg-transparent">
        <CardContent className="flex flex-col items-center justify-center p-12 text-center space-y-4">
           <div className="p-4 bg-white rounded-full shadow-sm">
              <Plus className="size-8 text-primary/40" />
           </div>
           <div>
              <p className="font-bold text-slate-500">Add a new automated routine</p>
              <p className="text-sm text-slate-400">Combine multiple zones into a single efficient schedule.</p>
           </div>
        </CardContent>
      </Card>
    </div>
